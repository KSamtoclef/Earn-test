-- Earn Chat production verification. Read-only. Run after all six production migrations.
with required_tables(name) as (values
 ...
 ('get_earnchat_business_config','get_earnchat_business_config()'),
 ...
 ('submit_earnchat_kyc','submit_earnchat_kyc(text,jsonb)'),
 ...
 ('mark_earnchat_presence_inactive','mark_earnchat_presence_inactive(text)'),
 ('get_public_earnchat_stats','get_public_earnchat_stats()'),
 ('record_earnchat_event','record_earnchat_event(text,text,text,jsonb)'),
 ('start_earnchat_qualification','start_earnchat_qualification(uuid)'),
 ('submit_earnchat_qualification','submit_earnchat_qualification(uuid,jsonb)'),
 ('earnchat_is_admin','earnchat_is_admin()'),
 ('admin_upsert_earnchat_task','admin_upsert_earnchat_task(uuid,jsonb)'),
 ('admin_delete_earnchat_task','admin_delete_earnchat_task(uuid)'),
 ('admin_review_task_claim','admin_review_task_claim(uuid,text,text)'),
 ('admin_reverse_task_claim','admin_reverse_task_claim(uuid,text)'),
 ('admin_reverse_earnchat_chat','admin_reverse_earnchat_chat(uuid,text)'),
...
union all
select 'function',name,to_regprocedure('public.'||signature) is not null from required_functions
...
select 'rls' check_name,tablename,rowsecurity from pg_tables
where schemaname='public' and (tablename='profiles' or tablename like 'earnchat_%') order by tablename;

select 'internal_helper_privilege_leak' check_name,p.proname,r.rolname
from pg_proc p join pg_namespace n on n.oid=p.pronamespace
cross join pg_roles r
where n.nspname='public'
 and p.proname in('earnchat_credit','mark_earnchat_active_day','refresh_earnchat_referral_qualification','evaluate_earnchat_level')
 and r.rolname in('anon','authenticated')
 and has_function_privilege(r.rolname,p.oid,'EXECUTE');

select 'duplicate_open_withdrawals' check_name,user_id,wallet_type,count(*)
from public.earnchat_withdrawals where status in('submitted','under_review','approved','processing')
...
with ledger_balances as (
 select user_id,wallet_type,coalesce(sum(case
  when entry_type in('credit','release') and status='approved' then amount
  when entry_type in('debit','hold','reversal') and status='approved' then -amount else 0 end),0) calculated
 from public.earnchat_ledger group by user_id,wallet_type
)
select 'wallet_mismatch' check_name,p.id,p.email,l.wallet_type,l.calculated,
 case when l.wallet_type='work' then p.work_available_balance else p.referral_available_balance end profile_balance
...
select 'pending_task_balance_mismatch' check_name,p.id,p.email,p.work_pending_balance,
 coalesce((select sum(c.reward_amount) from public.earnchat_task_claims c where c.user_id=p.id and c.status='pending'),0) calculated_pending
...
select 'pending_referral_balance_mismatch' check_name,p.id,p.email,p.referral_pending_balance,
 coalesce((select sum(r.reward_amount) from public.earnchat_referrals r where r.referrer_id=p.id and r.status='under_review'),0) calculated_pending
from public.profiles p
where p.referral_pending_balance<>coalesce((select sum(r.reward_amount) from public.earnchat_referrals r where r.referrer_id=p.id and r.status='under_review'),0);

select 'invalid_country_currency' check_name,id,email,country,currency from public.profiles
...
select 'invalid_chat_credit' check_name,c.id,c.user_id,c.reward_amount,l.amount ledger_amount
from public.earnchat_chat_sessions c left join public.earnchat_ledger l on l.source_type='chat' and l.source_id=c.id and l.entry_type='credit'
where c.status='approved' and coalesce(l.amount,-1)<>c.reward_amount;
select 'invalid_task_credit' check_name,c.id,c.user_id,c.reward_amount,l.amount ledger_amount
from public.earnchat_task_claims c left join public.earnchat_ledger l on l.source_type='task' and l.source_id=c.id and l.entry_type='credit'
where c.status='approved' and coalesce(l.amount,-1)<>c.reward_amount;

select 'configuration_summary' check_name,jsonb_build_object(
 'version',(select version from public.earnchat_business_settings where id=true),
