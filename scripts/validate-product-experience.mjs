...
if(!fail.length){
 ...
 // A noscript stylesheet fallback is intentionally allowed; it is inactive when JS is enabled.
 const activeHtml=html.replace(/<noscript>[\s\S]*?<\/noscript>/gi,'');
 if((activeHtml.match(/product-experience\.js/g)||[]).length!==1)fail.push('Experience runtime must be loaded exactly once');
 if((activeHtml.match(/product-experience\.css/g)||[]).length!==1)fail.push('Experience stylesheet must be loaded exactly once');
}
