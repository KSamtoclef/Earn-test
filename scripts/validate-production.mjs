...
const required=['index.html','assets/css/app.css','assets/css/routes.css','assets/css/experience-theme.css','assets/css/professional-ui.css','assets/css/member-motivation.css','assets/css/referral-priority.css','assets/css/level-chat-experience.css','assets/js/app-config.js','assets/js/supabase-client.js','assets/js/api.js','assets/js/router.js','assets/js/app.js','assets/js/admin/admin.js','assets/js/admin/core.js','assets/js/features/interaction-design.js','assets/js/features/draft-recovery.js','assets/js/features/member-motivation.js','assets/js/features/referral-priority.js','assets/js/features/level-journey.js','assets/js/features/guided-chat-experience.js','supabase/earnchat_production_install.sql','supabase/earnchat_kyc_bulk_admin_upgrade_20260730.sql','supabase/earnchat_level_chat_upgrade_20260731.sql','supabase/earnchat_production_verify.sql','supabase/PRODUCTION_RUN_ORDER.md'];
...
if(!loader.includes("RELEASE_VERSION='20260731-final-runtime-r2'"))fail.push('Final cache-busted runtime release is missing.');
for(const token of ['routes.css','professional-ui.css','member-motivation.css','referral-priority.css','level-chat-experience.css'])if(!loader.includes(token))fail.push(`Central customer stylesheet missing: ${token}`);
for(const token of ['await Promise.all','loadStyle(','link.addEventListener(\'load\'','link.addEventListener(\'error\''])if(!loader.includes(token))fail.push(`Styles are not guaranteed before rendering: ${token}`);
...
if(loader.indexOf('await Promise.all')>loader.indexOf("loadFeature('./assets/js/features/level-journey.js'"))fail.push('Customer features can render before their styles load.');
...
for(const token of ['#view-upgrade','upgrade-hero','upgrade-level-card','upgrade-requirement','route-level-strip','background:#fff!important','backdrop-filter:none!important','safe-area-inset-bottom','#view-upgrade .app-header{position:static!important','#view-upgrade .section-title{display:block','overflow-x:hidden','overflow-wrap:anywhere'])if(!levelCss.includes(token))fail.push(`Upgrade/mobile CSS missing: ${token}`);
...
console.log(`Checked ${required.length} required files, ${js.length} runtime modules, centrally awaited customer styles, responsive Upgrade layout, reliable progression loading, authoritative 45-second chat, valid actions, Admin accuracy and final database contracts.`);
