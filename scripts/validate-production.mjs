...
const config=read('assets/js/app-config.js'),loader=read('assets/js/supabase-client.js'),journey=read('assets/js/features/level-journey.js'),interaction=read('assets/js/features/interaction-design.js'),motivation=read('assets/js/features/member-motivation.js'),levelCss=read('assets/css/level-chat-experience.css'),api=read('assets/js/api.js'),core=read('assets/js/admin/core.js'),html=read('index.html');
if(!config.includes("'upgrade'"))fail.push('ROUTES does not contain upgrade.');
for(const token of ['level-journey','member-motivation','interaction-design'])if(!loader.includes(token))fail.push(`Feature loader missing: ${token}`);
...
if(/setInterval\([^,]+,\s*(?:350|500)\)/.test(read('assets/js/features/guided-chat-experience.js')))fail.push('High-frequency whole-page scanner remains.');
...
console.log(`Checked ${required.length} required files, ${js.length} runtime modules, dedicated Upgrade navigation, four-level progression, requirement actions, valid routes, solid mobile surfaces, Admin accuracy and final database contracts.`);
