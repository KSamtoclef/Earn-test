...
const requiredFiles=[
 ...
 'supabase/earnchat_production_install.sql','supabase/earnchat_production_verify.sql','supabase/earnchat_kyc_bulk_admin_upgrade_20260730.sql','supabase/earnchat_production_certification_upgrade_20260731.sql','supabase/PRODUCTION_RUN_ORDER.md'
];
...
const dynamicIds=new Set(['kyc-provider-modal','kyc-provider-close','kyc-provider-content','kyc-provider-message','task-proof-field','task-proof-value','example-label']);
...
const forbiddenRuntimeNames=['sponsored-visits-upgrade.js','linked-task-marketplace.js','auth-session-fix.js','earnchat-business-config.js','app-consistency-controller.js','earnchat-app-flow.js','earnchat-legacy-flow-bridge.js','earnchat-wallet-upgrade.js','enhancements.js','kyc-bulk-upgrade.js','kyc.js'];
...
if(/kyc(?:-bulk-upgrade|\.js)|admin\/enhancements/.test(loader))fail.push('Obsolete KYC or Admin override module is still loaded.');
...
const kyc=fs.readFileSync(path.join(root,'supabase','earnchat_kyc_bulk_admin_upgrade_20260730.sql'),'utf8');
const certification=fs.readFileSync(path.join(root,'supabase','earnchat_production_certification_upgrade_20260731.sql'),'utf8');
...
const requiredKycTokens=['get_earnchat_kyc_config','admin_update_earnchat_kyc_config','admin_bulk_review_earnchat_kyc','admin_bulk_review_task_claims','admin_bulk_update_user_control'];
const requiredCertificationTokens=['get_my_open_task_claim','get_my_open_chat_attempt','cancel_earnchat_chat_attempt','cancel_earnchat_task_claim','earnchat_one_started_task_per_user','failures'];
...
for(const token of requiredKycTokens)if(!kyc.includes(token))fail.push(`KYC upgrade SQL missing: ${token}`);
for(const token of requiredCertificationTokens)if(!certification.includes(token))fail.push(`Certification SQL missing: ${token}`);
...
console.log(`Checked ${requiredFiles.length} required files, ${runtimeFiles.length} runtime modules, core customer/Admin ownership, selectors, release versions, legacy removal and SQL recovery contracts.`);
