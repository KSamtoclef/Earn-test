...
export async function renderConfiguration(){
 ...
 try{
  ...
  $$('.admin-config-form',target).forEach(form=>{const button=$('button[type="submit"]',form);if(button)button.dataset.label=button.textContent;form.onsubmit=async event=>{event.preventDefault();const sectionName=form.dataset.section;setBusy(form,true);try{const payload=payloadFor(sectionName,form),response=sectionName==='business'?await api.adminUpdateBusiness(payload):await api.adminUpdateConfiguration(sectionName,payload);notifySaved(form,sectionName);setBusy(form,false,`Saved. Configuration version ${response?.configuration_version||response?.version||'updated'}.`);window.dispatchEvent(new CustomEvent('earnchat:config-updated',{detail:{section:sectionName,config:response,version:response?.configuration_version||response?.version||null,updated_at:response?.updated_at||null}}))}catch(error){setBusy(form,false,error.message||'Configuration could not be saved.')}}});
