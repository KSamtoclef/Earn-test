...
export async function renderConfiguration(){
 ...
 try{
  const config=normalizeBusinessConfig(await api.business(true));
  target.innerHTML=`<article class="card config-version-card"><span class="eyebrow">AUTHORITATIVE CONFIGURATION</span><h2>Platform control center</h2><p>Version <b>${esc(config.configuration_version||config.version)}</b>${config.updated_at?` · Updated ${new Date(config.updated_at).toLocaleString()}`:''}</p><p>Security infrastructure, routes, RLS and credentials remain code-controlled.</p></article>${generalForm(config)}${landingForm(config)}${chatForm(config)}${tasksForm(config)}${referralForm(config)}${withdrawalForm(config)}${kycForm(config)}${flagsForm(config)}${levelsForm(config)}`;
  $$('.admin-config-form',target).forEach(form=>{const button=$('button[type="submit"]',form);if(button)button.dataset.label=button.textContent;form.onsubmit=async event=>{event.preventDefault();const sectionName=form.dataset.section;setBusy(form,true);try{const response=await api.adminUpdateConfiguration(sectionName,payloadFor(sectionName,form));notifySaved(form,sectionName);setBusy(form,false,`Saved. Configuration version ${response?.configuration_version||response?.version||'updated'}.`);window.dispatchEvent(new CustomEvent('earnchat:config-updated',{detail:{section:sectionName,config:response,version:response?.configuration_version||response?.version||null,updated_at:response?.updated_at||null}}))}catch(error){setBusy(form,false,error.message||'Configuration could not be saved.')}}});
