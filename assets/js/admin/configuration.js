...
function chatForm(config){const c=config.chat;return section('chat','Guided Chat','Timing, reply quality, recovery, prompts and partners.',`
 ...
 `)}
function tasksForm(config){const c=config.tasks;return section('tasks','Sponsored Visits','Sponsored-visit lifecycle and customer status wording.',`
 ...
 <div class="quick-grid">${input('default_attempt_expiry_minutes','Default attempt expiry minutes',c.default_attempt_expiry_minutes,{type:'number',min:1,max:1440})}${input('featured_task_limit','Featured visit limit',c.featured_task_limit,{type:'number',min:0,max:50})}</div>
...
function payloadFor(section,form){const data=new FormData(form),bool=boolValue(data);switch(section){
 ...
 case'feature_flags':return{...Object.fromEntries([...data.keys()].map(key=>[key,bool(key)])),tasks:false};
