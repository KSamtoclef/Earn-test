...
function updateLanding(){
  ...
  if(hero){
    ...
    if(paragraph)paragraph.innerHTML='Complete approved chats and linked tasks at your level.<br><span id="earnchat-earning-promise">Earn up to '+copy.earning+' during your 5-day activity cycle</span>';
  }
  var stats=landing.querySelectorAll('.big-stats .bs-card');
  if(stats.length>=3){
    stats[0].innerHTML='<div class="bs-val">4</div><div class="bs-lbl">Starter chats daily</div>';
    stats[1].innerHTML='<div class="bs-val">5 Days</div><div class="bs-lbl">Guided activity cycle</div>';
    stats[2].innerHTML='<div class="bs-val">More</div><div class="bs-lbl">Tasks and referrals</div>';
  }
  ...
  if(steps.length>=3){steps[0].textContent='Create your free account';steps[1].textContent='Complete chats and linked tasks';steps[2].textContent='Unlock higher earning levels'}
}
...
window.addEventListener('earnchat:state-updated',refresh);
window.addEventListener('earnchat:business-ready',refresh);
window.addEventListener('storage',function(event){if(event.key==='earnchat-country')refresh()});
