...
function homeClarity(state=currentState){
 ...
 const tasks=Number(profile.approved_tasks_count??state?.approved_tasks_count??0);
 const pending=Number(wallet.work_pending??0)+Number(wallet.referral_pending??0);
 card.innerHTML=`<header><div><h3>Your account at a glance</h3><p>See what is ready, what is pending, and what helps you reach the next level.</p></div></header><div class="member-clarity-grid"><div><small>Pending review</small><b>${pending?pending.toLocaleString():'Check activity history'}</b></div><div><small>Activity points</small><b>${points.toLocaleString()} AP</b></div><div><small>Approved tasks</small><b>${tasks.toLocaleString()} · ${days.toLocaleString()} active day${days===1?'':'s'}</b></div></div>`;
}
...
function ensureActivityGuide(containerId,type){
 ...
 if(!guide){
  ...
  guide.innerHTML=`<h2>${visit?'Sponsored visits':'Tasks'} made simple</h2><p>${visit?'Open the approved partner page, stay for the required time, then return to submit.':'Read the guide, complete the approved activity, and submit only the proof requested.'}</p><div class="activity-guide-grid"><span>1 · Check reward and time</span><span>2 · Follow the approved guide</span><span>3 · Return and submit</span></div>`;
...
function upgradeClarity(){
 ...
 banner.innerHTML='<h2>Earn your next level</h2><p>Your level increases through approved chats, tasks, active days and genuine referrals. There is nothing to buy.</p><strong>Keep working—the dashboard will show exactly what remains.</strong>';
 const header=$('.app-header',container);if(header)header.insertAdjacentElement('afterend',banner);else container.prepend(banner);
}

function refreshExperience(){addLandingExperience();homeClarity();ensureActivityGuide('#task-list','task');ensureActivityGuide('#visit-list','visit');referralClarity();upgradeClarity()}

for(const id of['#task-list','#visit-list']){const host=$(id);if(host)new MutationObserver(()=>ensureActivityGuide(id,id.includes('visit')?'visit':'task')).observe(host,{childList:true})}
