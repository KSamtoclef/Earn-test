(function(){
  ...
  window.EARNCHAT_BUILD='2026-07-30-consistency-1';
  ...
  function ensureAppExperience(){
    ...
    appExperiencePromise=Promise.all([
      loadRuntimeScript('./assets/js/core-flow-upgrade.js?v=20260730-4','data-earn-chat-core-flow'),
      loadRuntimeScript('./assets/js/professional-five-day-upgrade.js?v=20260730-5','data-earn-chat-professional-five-day'),
      loadRuntimeScript('./assets/js/sponsored-visits-upgrade.js?v=20260730-4','data-earn-chat-sponsored-visits'),
      loadRuntimeScript('./assets/js/share-message-upgrade.js?v=20260730-1','data-earn-chat-share-upgrade')
    ]).then(function(){window.dispatchEvent(new CustomEvent('earnchat:app-ready'));});
  ...
  function openPage(pageId){
    ensureAppExperience();
    if(typeof window.pg==='function'){window.pg(pageId);return;}
  ...
  function stabilisePublicUi(){
    if(!document.getElementById('earnchat-public-stability')){
  ...
  // Lightweight shared rules and visual system load before authentication.
  loadRuntimeScript('./assets/js/app-consistency-controller.js?v=20260730-1','data-earn-chat-consistency');
  ...
  window._supa=window._supa||window.supabase.createClient(window.SUPABASE_URL,window.SUPABASE_KEY,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:true,storageKey:'earn-chat-auth-v1'}});

  function showAuthStatus(message,isError){
    var box=document.getElementById('earnchat-auth-runtime-status');
    if(!box){box=document.createElement('div');box.id='earnchat-auth-runtime-status';box.style.cssText='position:fixed;left:12px;right:12px;top:12px;z-index:100001;padding:12px 14px;border-radius:13px;font:600 12px/1.45 Inter,sans-serif;display:none';document.body.appendChild(box);}
    box.textContent=message||'';box.style.display=message?'block':'none';box.style.background=isError?'#fff0f2':'#eef7ff';box.style.color=isError?'#a52d3d':'#24598f';box.style.border='1px solid '+(isError?'#efb9c1':'#bfdcff');
  ...
  async function restoreEarnChatSession(){
    try{
      ...
      window.saveState();window.updateUI();window.dispatchEvent(new CustomEvent('earnchat:state-updated'));
