...
let isAuthenticated=()=>false,onRoute=()=>{},scheduled=false;
...
export function configureRouter({isAuthenticated:authCheck,routeHandler}){isAuthenticated=authCheck;onRoute=routeHandler;window.addEventListener('hashchange',scheduleResolve);window.addEventListener('popstate',scheduleResolve)}
export function route(){return clean()}
export function navigate(name,replace=false){if(!ROUTES.includes(name))name=isAuthenticated()?'home':'landing';const hash='#/'+name;if(location.hash===hash){scheduleResolve();return}if(replace)history.replaceState({},'',hash);else history.pushState({},'',hash);scheduleResolve()}
