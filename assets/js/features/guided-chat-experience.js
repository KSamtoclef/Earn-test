...
let completing=false,chatInterval=null,patchTimer=null;
...
function renderClock(){if(route()!=='chat')return;const saved=savedChat(),timer=$('#chat-timer'),complete=$('#chat-complete'),ready=$('#chat-ready');if(!timer||!complete)return;const startedAt=Number(saved?.startedAt||Date.now()),elapsed=Math.max(0,Math.floor((Date.now()-startedAt)/1000)),remaining=Math.max(0,REQUIRED_SECONDS-elapsed),replyCount=Array.isArray(saved?.replyTexts)?saved.replyTexts.length:0;timer.textContent=`${formatTime(elapsed)} / 00:45`;timer.dataset.authoritativeTimer='45';if(replyCount===4&&ready){ready.classList.remove('hidden');complete.disabled=remaining>0||completing;complete.textContent=remaining>0?`Ready in ${remaining}s`:'Complete conversation'}}
...
async function complete(){
 ...
 completing=true;const button=$('#chat-complete');if(button){button.disabled=true;button.textContent='Verifying…'}
...
function enhance(){patchWording();if(route()!=='chat')return;document.body.classList.add('compact-guided-chat');const completeButton=$('#chat-complete');if(completeButton)completeButton.dataset.authoritativeChat='45';const suggestions=$('#chat-suggestions');if(suggestions){suggestions.setAttribute('aria-label','Quick reply suggestions');const buttons=[...suggestions.querySelectorAll('button')];buttons.slice(2).forEach(button=>button.remove())}renderClock()}
