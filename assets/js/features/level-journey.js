...
const CACHE_MS=8000;
let loading=false,lastState=null,lastLoadedAt=0,timer=null;
...
function renderCompact(state){const data=progressData(state);const home=$('#view-home .container');if(home){let card=$('#home-upgrade-summary');if(!card){card=document.createElement('section');card.id='home-upgrade-summary';card.className='home-upgrade-summary';const balance=$('.balance-card',home);balance?.insertAdjacentElement('afterend',card)}const missing=data.requirements.filter(x=>!x.done).slice(0,3);card.innerHTML=`<header><div><span class="eyebrow">LEVEL PROGRESS</span><h2>${data.current} → ${data.next}</h2></div><b>${data.score}%</b></header><div class="progress"><i style="width:${data.score}%"></i></div><ul>${missing.map(x=>`<li><span>○</span>${x.key==='account-days'?`${Math.max(0,x.target-x.current)} account days remaining`:`${Math.max(0,x.target-x.current)} more ${x.label.toLowerCase()}`}</li>`).join('')||'<li><span>✓</span>All next-level requirements completed</li>'}</ul><button class="primary" data-go="upgrade" type="button">View upgrade journey</button>`}
 const route=document.body.dataset.route;const target=['earn','tasks','visits','referrals','profile'].includes(route)?$(`#view-${route} .container`):null;if(target){let strip=$(`#view-${route} .route-level-strip`);if(!strip){strip=document.createElement('button');strip.type='button';strip.className='route-level-strip';strip.dataset.go='upgrade';const header=$('.app-header',target);header?.insertAdjacentElement('afterend',strip)}strip.innerHTML=`<span><b>${data.current}</b><small>${data.score}% toward ${data.next}</small></span><strong>View upgrade →</strong>`}
}
function makeLevelControls(){
 $$('.app-header .pill').forEach(span=>{if(span.tagName==='BUTTON')return;const button=document.createElement('button');button.type='button';button.className=`${span.className} level-pill-button`;button.dataset.go='upgrade';button.textContent=span.textContent||'Starter';button.setAttribute('aria-label',`View ${button.textContent.trim()} level progress`);span.replaceWith(button)});
 const homeLevel=$('#home-level');if(homeLevel&&homeLevel.dataset.levelReady!=='1'){homeLevel.dataset.levelReady='1';homeLevel.setAttribute('role','button');homeLevel.setAttribute('tabindex','0');homeLevel.setAttribute('aria-label','View level progress');homeLevel.onclick=()=>location.hash='#/upgrade';homeLevel.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();location.hash='#/upgrade'}}}
 const progress=$('#view-progress');if(progress){progress.dataset.go='upgrade';progress.textContent='Upgrade progress →'}
}
function unlockLockedTaskButtons(){$$('.task-start[disabled]').forEach(button=>{button.disabled=false;button.classList.add('locked-level-action');button.dataset.go='upgrade';button.removeAttribute('data-id');button.setAttribute('aria-label',`${button.textContent}. View upgrade requirements`)})}
async function state(force=false){if(!force&&lastState&&Date.now()-lastLoadedAt<CACHE_MS)return lastState;if(loading)return lastState;loading=true;try{lastState=await api.state();lastLoadedAt=Date.now();return lastState}finally{loading=false}}
async function enhance(force=false){ensureShell();makeLevelControls();unlockLockedTaskButtons();const route=document.body.dataset.route||location.hash.replace('#/','');if(!['home','earn','tasks','visits','upgrade','referrals','profile'].includes(route))return;try{const data=await state(force);if(!data)return;renderCompact(data);if(route==='upgrade')renderUpgrade(data)}catch{const host=$('#upgrade-content');if(route==='upgrade'&&host)host.innerHTML='<div class="form-message error show">Level progress could not load. Check your connection and try again.</div>'}}
...
window.addEventListener('earnchat:member-state',event=>{lastState=event.detail;lastLoadedAt=Date.now();schedule(0,false)});
window.addEventListener('hashchange',()=>schedule(60,false));
window.addEventListener('pageshow',()=>schedule(80,false));
document.addEventListener('visibilitychange',()=>{if(!document.hidden)schedule(80,true)});
