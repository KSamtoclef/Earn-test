...
function restore(form,config){if(form.dataset.draftReady==='1')return;form.dataset.draftReady='1';statusNode(form);let draft=null;try{draft=JSON.parse(store(config.storage).getItem(key(form))||'null')}catch{}if(!draft?.values)return;for(const el of controls(form)){const name=el.id||el.name,item=draft.values[name];if(!item)continue;if(el.type==='checkbox'||el.type==='radio')el.checked=!!item.checked;else if(String(el.value??'').trim()===''||form.id==='kyc-config-form'||form.id==='business-form')el.value=item.value??'';el.dispatchEvent(new Event('change',{bubbles:true}))}const when=new Date(draft.savedAt||Date.now()).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'});setStatus(form,`Draft restored from ${when}`)}
